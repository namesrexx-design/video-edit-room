...
**Correction and release gates (18 September):** The existing site destination is `https://biz-box.io/links`, as documented in the Bizbox repo and guides. Remove the unverified domain inferred from the earlier dictated message from all proposed destinations. No domain migration, canonical-link replacement, or social-bio change is authorized by this plan. The scheduler's missing-key fail-open behavior must be fixed and verified before enabling agent access. Rexx's updated starting cadence is 3–5 posts per day, then scale based on what performs. This supersedes the earlier 10/day assumption. Track original content items and platform deliveries separately; a per-platform multiplier is not yet specified. Do not commit to a paid plan without the actual destination mix and approved cost.
...
Budget sizing for #25: Rexx now sets the initial cadence at 3–5 posts/day, with scaling only after reviewing what works and what does not. This replaces the earlier 10/day planning assumption. On a 30-day basis, 3–5 total deliveries/day is 90–150 deliveries/month; if each of 3–5 original pieces is distributed to seven destinations, it is 630–1,050 deliveries/month. These are alternative budget scenarios, not a confirmed per-platform schedule. Keep original items and destination deliveries as separate counters, and establish the actual destination mix before a cost commitment. Track results by platform, format, hook, and offer using available retention/completion, saves/shares, link visits, and downstream signup/booking/sales data. Review comparable results regularly and increase volume after the evidence supports it; no automatic ramp is authorized. Model uploads, reads, polling, and retries separately. Native API quotas/reviews are not broker subscription fees. Claude must produce current quotes for the actual volume and connected accounts before recommending a purchase; no guaranteed review timeline is assumed.
