...
def motion():
 ...
 save(col,P/'source/motion-column.svg',1080,step*len(a['segments']))
 ...
 proj=P;md=proj/'projects/garage-dream/deliveries/CONTENT';md.mkdir(parents=True,exist_ok=True)
 run(['ffmpeg','-v','error','-y','-loop','1','-framerate','24','-i',P/'source/motion-base.png','-loop','1','-framerate','24','-i',P/'source/motion-column.png','-filter_complex_threads','1','-filter_complex',fg,'-map','[v]','-frames:v',frames,'-an','-c:v','libx264','-threads','2','-preset','fast','-crf','18','-movflags','+faststart',md/'PILE-ESCALATION-PICTURE.mp4'])
 run(['ffmpeg','-v','error','-y','-i',P/'audio'/f'{tim["generationId"]}.mp3','-af',f'loudnorm=I=-18:TP=-2:LRA=8,adelay=350|350,apad,atrim=duration={dur}','-ar','48000','-ac','2','-c:a','pcm_s24le',md/'PILE-ESCALATION-VOICE.wav'])
 name='PILE-ESCALATION-FINAL-20260917';u='general-pile'
 cut={'format':2,'name':name,'savedAt':'2026-09-17T00:00:00Z','output':{'width':1080,'height':1920},'note':'General reusable content template, centered portrait, no title/name labels. One complete namesrexx eleven_v3 take.','v1':[{'id':u,'u':u,'scene':'GENERAL','folder':'repo-deliveries','file':'CONTENT/PILE-ESCALATION-PICTURE.mp4','start':0,'end':frames,'mute':True}],'v2':[],'a2':[{'id':u+'-audio','u':u+'-audio','link':u,'folder':'repo-deliveries','file':'CONTENT/PILE-ESCALATION-VOICE.wav','start':0,'end':frames,'at':0,'gain':0}]}
