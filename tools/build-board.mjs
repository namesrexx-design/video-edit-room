...
const scenes = sb.scenes.map(s => {
  ...
  for (const c of invClips.filter(c => new RegExp('(^|[^A-Z0-9])' + s.id + '([^0-9A-Z]|$)', 'i').test(c.title || ''))) { const th = EDITOR + '/thumbs/' + c.id + '.jpg'; if (fs.existsSync(th)) stills.push(tile(th, { videoUrl: fs.existsSync(path.resolve('projects/garage-dream/deliveries/CLIPS/' + c.id + '.mp4')) ? '../projects/garage-dream/deliveries/CLIPS/' + c.id + '.mp4' : null, group: (s.archivedClipIds || []).includes(c.id) ? 'archived' : (s.approvedClipIds || []).includes(c.id) ? 'approved' : c.group === 'owner' ? 'cut' : 'pending', approved: (s.approvedClipIds || []).includes(c.id), id: c.id, title: (c.group === 'owner' ? 'Owner upload · ' : c.group === 'lipsync' ? 'Lip-sync clip · ' : 'Higgsfield clip · ') + c.id + ' · ' + c.seconds + ' s', role: 'clip', approved: c.group === 'owner', status: c.title, file: c.file })); }
  const kw = new RegExp('\\b' + s.id + '\\b', 'i');
  const audio = audioBin.filter(a => kw.test(a.title) || kw.test(a.file) || (s.id === 'S19' && /bart/i.test(a.title)) || (s.id === 'S16' && /whisper|bugatti intro/i.test(a.title)) || (s.id === 'S07' && /waymo/i.test(a.title)) || (s.id === 'S18' && /snore|mosquito/i.test(a.title)) || (s.id === 'S17' && /song|bugatti phrase/i.test(a.title))).map(a => ({ id: a.id, title: a.title, group: a.group, audioUrl: fs.existsSync(path.resolve('projects/garage-dream/deliveries/AUDIO/' + a.id + '.mp3')) ? '../projects/garage-dream/deliveries/AUDIO/' + a.id + '.mp3' : null, seconds: a.seconds, date: (a.local && dateOf(a.local)) || null }));
...
const FILMS = fs.existsSync(FILMDIR) ? fs.readdirSync(FILMDIR).filter(f => /.mp4$/i.test(f)).sort().reverse() : [];
const FILM = FILMS.length ? { file: FILMS[0], name: FILMS[0].replace(/.mp4$/i, '').replace(/-/g, ' '), urls: ['../projects/garage-dream/deliveries/FILM/' + FILMS[0], FILMS[0]], bytes: fs.statSync(path.join(FILMDIR, FILMS[0])).size } : null;
...
const LIBRARY = { clips: invClips.filter(c => fs.existsSync(path.resolve('projects/garage-dream/deliveries/CLIPS/' + c.id + '.mp4'))).map(c => ({ id: c.id, title: c.title || c.file, date: c.date, group: c.group, videoUrl: '../projects/garage-dream/deliveries/CLIPS/' + c.id + '.mp4' })), audio: audioBin.filter(a => fs.existsSync(path.resolve('projects/garage-dream/deliveries/AUDIO/' + a.id + '.mp3'))).map(a => ({ id: a.id, title: a.title, seconds: a.seconds, audioUrl: '../projects/garage-dream/deliveries/AUDIO/' + a.id + '.mp3' })) };
