...
const scenes = sb.scenes.map(s => {
  ...
  for (const c of invClips.filter(c => new RegExp('(^|[^A-Z0-9])' + s.id + '([^0-9A-Z]|$)', 'i').test(c.title || ''))) { const th = EDITOR + '/thumbs/' + c.id + '.jpg'; if (fs.existsSync(th)) stills.push(tile(th, { group: (s.archivedClipIds || []).includes(c.id) ? 'archived' : (s.approvedClipIds || []).includes(c.id) ? 'approved' : c.group === 'owner' ? 'cut' : 'pending', approved: (s.approvedClipIds || []).includes(c.id), id: c.id, title: (c.group === 'owner' ? 'Owner upload · ' : c.group === 'lipsync' ? 'Lip-sync clip · ' : 'Higgsfield clip · ') + c.id + ' · ' + c.seconds + ' s', role: 'clip', approved: c.group === 'owner', status: c.title, file: c.file })); }
