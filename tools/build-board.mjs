...
const resolve = (p, base) => { if (!p || typeof p !== 'string') return null; p = mapPath(p); for (const b of [base, W + '/scene-board', W, MEDIA_ROOT]) { const f = path.resolve(b, p); if (fs.existsSync(f) && fs.statSync(f).isFile()) return f; } return null; };
...
for (const g of lock.groups || []) for (const it of g.items || []) { if (isPropNotCharacter(it) || it.ownerRejected) continue;   // owner rejected for good (2026-09-16): file stays on disk, tile leaves the board
  const c = it.character || 'Other'; (chars[c] ||= { name: c, refs: [] }); chars[c].refs.push(tile(resolve(it.path, W), { id: it.id, title: it.title, role: clean(it.role), notes: clean(it.notes), approved: !!it.approved, status: clean(it.status), group: g.title, view: viewOf(it.title + ' ' + (it.role || '')) })); }
...
const characters = [...order.filter(k => chars[k]).map(k => chars[k]), ...Object.keys(chars).filter(k => !order.includes(k)).map(k => chars[k])].map(c => { const m = pack('char-' + c.name.replace(/\W+/g, '_'), c.refs.map(t => t.src), 'port'); const seenRef = new Set(); const refs = bind(c.refs, m).filter(r => { const k = (r.src || r.id) + '|' + r.id; if (seenRef.has(k)) return false; seenRef.add(k); return true; }); const cur = refs.filter(r => r.approved), rest = refs.filter(r => !r.approved);
