...
for (const g of lock.groups || []) for (const it of g.items || []) { const c = it.character || 'Other'; (chars[c] ||= { name: c, refs: [] }); chars[c].refs.push(tile(resolve(it.path, W), { id: it.id, title: it.title, role: clean(it.role), notes: clean(it.notes), approved: !!it.approved, status: clean(it.status), group: g.title, view: viewOf(it.title + ' ' + (it.role || '')) })); }
...
const characters = [...order.filter(k => chars[k]).map(k => chars[k]), ...Object.keys(chars).filter(k => !order.includes(k)).map(k => chars[k])].map(c => { const m = pack('char-' + c.name.replace(/\W+/g, '_'), c.refs.map(t => t.src), 'port'); const refs = bind(c.refs, m); return { name: c.name, status: c.status, voice: c.voice, continuity: c.continuity, current: refs.filter(r => r.approved), archive: refs.filter(r => !r.approved) }; });
...
const sets = SETS_ORDER.map(([name, blurb]) => {
  ...
  return { name, blurb, scenes: inSet.map(s => s.id), current: bind(current, m), archive: bind(archive, m) };
});
