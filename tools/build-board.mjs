...
const scenes = sb.scenes.map(s => {
  ...
  for (const c of invClips.filter(c => new RegExp('(^|[^A-Z0-9])' + s.id + '([^0-9A-Z]|$)', 'i').test(c.title || ''))) { const th = EDITOR + '/thumbs/' + c.id + '.jpg'; if (fs.existsSync(th)) stills.push(tile(th, { group: (s.archivedClipIds || []).includes(c.id) ? 'archived' : c.group === 'owner' ? 'cut' : 'pending', id: c.id, title: (c.group === 'owner' ? 'Owner upload · ' : c.group === 'lipsync' ? 'Lip-sync clip · ' : 'Higgsfield clip · ') + c.id + ' · ' + c.seconds + ' s', role: 'clip', approved: c.group === 'owner', status: c.title, file: c.file })); }
  ...
  return { id: s.id, title: s.title, inFilm: s.assembly?.active !== false, removedWhy: s.assembly?.active === false ? clean(s.assembly.reason) : "", line: clean(s.line), status: clean(s.status), nextAction: clean(s.finalization?.nextAction), gate: clean(s.productionGate?.reason), video: s.fullVideo ? path.basename(s.fullVideo) : null, editStatus: clean(s.editStatus || s.latestWorkingEdit?.status),
