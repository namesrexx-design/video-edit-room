...
const V74_DATE = '2026-09-14';  // the base cut; anything dated before this is older than the film as it stands
const dateOf = f => { try { return fs.statSync(f).mtime.toISOString().slice(0, 10); } catch (e) { return null; } };

// scenes
const scenes = sb.scenes.map(s => { const f = resolve(s.image, W + '/scene-board'); return ({ id: s.id, title: s.title, line: clean(s.line), status: clean(s.status), still: thumb(f), stillDate: dateOf(f), stillStale: !!(f && dateOf(f) < V74_DATE), v74: s.v74?.episodeSeconds ? { start: s.v74.episodeSeconds[0], end: s.v74.episodeSeconds[1] } : (s.v74?.inCut === false ? { inCut: false, note: s.v74.note } : null), phase: s.episode || '' }); });

// characters: reference-lock items grouped by character + continuity notes + voices
const chars = {}; const order = ['Rexx', 'Homer', 'Apu', 'Bart'];
for (const g of lock.groups || []) for (const it of g.items || []) { const c = it.character || 'Other'; (chars[c] ||= { name: c, refs: [] }); chars[c].refs.push({ id: it.id, title: it.title, role: clean(it.role), notes: clean(it.notes), approved: !!it.approved, status: clean(it.status), group: g.title, img: thumb(resolve(it.path, W)), date: dateOf(resolve(it.path, W)) }); }
...
const dh = sb.characterReferences?.dreamHomer; if (dh) (chars.Homer ||= { name: 'Homer', refs: [] }).refs.push({ id: dh.id, title: dh.title, role: clean(dh.role), notes: clean(dh.notes), approved: !!dh.approved, status: clean(dh.status), group: 'Dream look', img: thumb(resolve(dh.path, W)), date: dateOf(resolve(dh.path, W)) });
...
const addObj = (title, p, role, status, extra = {}) => { const f = resolve(p, W); objects.push({ title, role: clean(role), status: clean(status), img: thumb(f), date: dateOf(f), ...extra }); };
...
const board = { v74Date: V74_DATE, source: 'Codex scene-board/storyboard.json + continuity-reference-pack/reference-lock.json', builtAt: new Date().toISOString(), version: sb.version, currentCut: sb.currentFullCut?.title, scenes, characters, objects, lockStatus: clean(lock.status), blockers: (lock.blockers || []).map(clean) };
