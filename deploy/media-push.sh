...
rclone copy "$MEDIA" "$B/media" $DRY "${EXCL[@]}" --transfers 8 --checkers 16 --progress --stats-one-line
rclone copy "$CODEX" "$B/codex-workspace" $DRY "${EXCL[@]}" --exclude "*.bak*" --transfers 8 --checkers 16 --progress --stats-one-line
...
rclone copy "$EDITOR" "$B/editor" $DRY "${EXCL[@]}" --transfers 8 --checkers 16 --progress --stats-one-line
